...
class ConvNormReLU(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel: int = 3, stride: int = 1, padding: int = None,
                 ...
                 a_bits: int = 8, existing_quant_layer: qnn.QuantReLU = None):
        """
        ...
        # Addition of residual branch requires the same quantizer (scaling factors) on both operands
        if existing_quant_layer is not None:
            self.relu = existing_quant_layer
        else:
            self.relu = qnn.QuantReLU(bit_width=a_bits, return_quant_tensor=True)
...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel: int = 3, stride: int = 1, padding: int = None,
                 w_bits: int = 8, a_bits: int = 8, previous_quant_layer: qnn.QuantReLU = None):
        """
        ...
        if self.use_res_connect:
            self.pw = ConvNormReLU(in_channels=in_channels, out_channels=out_channels, kernel=1, stride=1,
                                   w_bits=w_bits, a_bits=a_bits, existing_quant_layer=previous_quant_layer)
        else:
            self.pw = ConvNormReLU(in_channels=in_channels, out_channels=out_channels, kernel=1, stride=1,
                                   w_bits=w_bits, a_bits=a_bits)
...
class BacalhauNetV1(nn.Module):
    def __init__(self, config: BacalhauNetConfig):
        ...
        self.hardtan = qnn.QuantHardTanh(min_val=-2, max_val=3, bit_width=config.hardtanh_bit_width, return_quant_tensor=True)
        ...
        for i in range(1, len(config.layers)):
            conv_layers.append(DepthwiseSeparableConv(in_channels=conv_layers[-1].out_channels,
                                                      ...
                                                      previous_quant_layer=conv_layers[-1].pw.relu))
